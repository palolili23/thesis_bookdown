...
\small
\noindent 
_This chapter has been published as_: Caniglia, E.C., Rojas-Saunero L.P., Hilal S., Licher S., Logan R., Stricker B., Ikram M. A., Swanson S.A. Emulating a target trial of statin use and risk of dementia using cohort data. *Neurology*. 2020;95(10):e1322-e1332.
\newpage
\normalsize

```{r call_tables_statins, echo = FALSE, warning=FALSE, message = FALSE}
library(knitr)
library(kableExtra)
library(magrittr)
library(dplyr)
source(here::here("tables", "statins_tables.R"))
...
We begin by describing the protocol of our target trial to estimate the effect of statins on incident dementia; subsequently, we describe how we emulate this target trial in our data. A summary of these protocol components can be found in Table \@ref(tab:statinst1).
...
```{r statinst1, echo= FALSE, warning = FALSE, results='asis'}
table2.1 %>%
  kable(
        format = 'latex',
        booktabs = TRUE,
        caption = '(ref:statinst1)',
        align = "lll",
        escape = F) %>%  
  kable_styling(
    full_width = F,
  ...
  footnote(general = "*Serious illness defined as cancer or heart disease. Following a serious illness, participants may or may not take statin therapy, as guided by their physicians.", threeparttable = T) 
```
(ref:statinst1) **Protocol of target trial and emulation using observational data, Rotterdam Study -- I and II**

\newpage

## Results

From January 1993 to December 2007 a total of 180 trials were conducted. Of 1,578,655 potential person-trials (10,942 individuals), 233,526 person-trials (6,373 individuals) met the eligibility criteria. There were 622 initiators and 232,904 non-initiators (Figure \@ref(fig:statinsf1)). Across the 180 months of trials, the mean number of participants in a given month was 1,297 with a mean number of 3.5 initiators. The trial beginning in March 2006 had the most initiators (15 initiators, 1,915 noninitiators; Supplementary information, Table \@ref(tab:statinst5)). Initiators and non-initiators were similar in age, MMSE, educational attainment. Compared with non-initiators, initiators had higher baseline total cholesterol measurements and systolic blood pressure measurements and were more likely to be current or former smokers, have a history of heart disease, stroke, and diabetes, and use other prescription medications (Table \@ref(tab:statinst2)).
...
```{r statinsf1, echo=FALSE, fig.cap = '(ref:statinsf1)', auto_pdf = TRUE, fig.show='asis', fig.align='center', out.width = "85%"}
knitr::include_graphics(here::here("figs", "statinsf1_test.png"))
```
(ref:statinsf1) Flowchart of person-trials in the modified intention-to-treat analysis

\newpage

```{r statinst2, echo= FALSE, warning = FALSE, results='asis'}
table2.2 %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = '(ref:statinst2)',
    align = "lcc",
    escape = F) %>% 
  kable_styling(
    full_width = F,
    ...
    column_spec(c(2,3), width = "2.2cm") %>%
  add_indent(c(7:11, 13:15, 17:19, 23:25, 29:31, 33:35, 37:45))%>% 
  footnote(general = legend2.2, threeparttable =T)
```
(ref:statinst2) Characteristics of eligible individuals at the start of the trial's follow-up, Rotterdam-Study I and II

Figure \@ref(fig:statinsf2) shows the modified intention-to-treat 10-year dementia-free and dementia-free survival curves by statin initiation. Comparing statin initiation with no initiation, the 10-year risk differences (95% CI) were -0.1% (-2.3%, 1.8%) for dementia and 0.3% (-2.7%, 3.3%) for dementia or death (Table \@ref(tab:statinst3)). Unadjusted estimates were similar for dementia but larger for dementia or death (data not shown).

```{r statinst3, echo= FALSE, warning = FALSE, results='asis'}
table2.3 %>%
  select(1:4) %>%
  rename(`Risk % (95%CI)` = `Risk %(95%CI)_1`,
         `Risk Difference % (95%CI)` = `Risk Difference(%)_1`,
         Outcome = `Risk`) %>% 
  kable(booktabs = TRUE,
        caption = '(ref:statinst3)',
        align = "llcc") %>%
  kable_styling(
    full_width = F,
    position = 'center',
...
#### Participants per person-trial {-}

```{r statinst5, echo= FALSE, warning = FALSE, results='asis'}
table2e1 %>%
  kable(booktabs = TRUE,
        longtable = TRUE,
        caption = '(ref:statinst5)',
        align = "cccc") %>%
  kable_styling(
    full_width = F,
    position = 'center',
    ...
    font_size = 9) %>% 
      row_spec(0, 0, bold = TRUE)
```
(ref:statinst5) Number of participants and initiators in each non-randomized ‘trial,’ Rotterdam Study – I and II

\blandscape

#### Per-protocol effect: Sensitivity and Subgroup analysis {-}

```{r statinst6, echo= FALSE, warning = FALSE, results='asis'}
table2e2 %>%
  select(1:6) %>%
  rename(`No statins \\% (95\\%CI)` = `No statins %(95%CI)_1`,
         `Statins \\% (95\\%CI)` = `Statins %(95%CI)_1`) %>%
  kable(
    format = 'latex',
    escape = F,
    booktabs = TRUE,
    caption = '(ref:statinst6)',
    align = "lccccc"
  ) %>%
  kable_styling(
    full_width = F,
  ...
  footnote(general = "* non-statin cholesterol-lowering medications, diuretics, beta-blockers, calcium-blockers, ace-inhibitors, NSAIDs, or aspirin", threeparttable = T) %>%
  column_spec(1, width = "5cm") %>%
  column_spec(c(2, 3), width = "1.2cm") %>%
  column_spec(c(4, 5, 6), width = "2.2cm") %>%
  row_spec(0, 0, bold = TRUE)
```
(ref:statinst6) Five-year risk and risk difference of dementia or death under both treatment arms

\newpage

```{r statinst7, echo= FALSE, warning = FALSE, results='asis'}
table2e2 %>%
  select(1:3, 7:9) %>% 
  rename(`No statins \\% (95\\%CI)` = `No statins %(95%CI)_2`,
         `Statins \\% (95\\%CI)` = `Statins %(95%CI)_2`) %>%
  kable(
    format = 'latex',
    escape = F,
        booktabs = TRUE,
    caption = '(ref:statinst7)',
    align = "lcccc"
  ) %>%
  kable_styling(
    full_width = F,
  ...
  row_spec(0, 0, bold = TRUE)
```
(ref:statinst7) Ten-year risk and risk difference of dementia or death under both treatment arms
